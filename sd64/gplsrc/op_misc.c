 ...
 */
  ...
  if (!sdsendmail(sender, recipients, cc_recipients, bcc_recipients, subject,
                  text, attachments)) {
    /* process.status and possibly process.os_error set by sdsendmail */
  }
...
char* day_to_ddmmmyyyy(int32_t day_no) /* sd format day number */
{
  int16_t year;
  int16_t mon;
  int16_t day;
  ...
  #define date_stringsz 12  /* 11 char + \0 */
  static char date_string[date_stringsz];

  day_to_dmy(day_no, &day, &mon, &year, &day_of_year);

  /* Build result string */
  /*  20240122  mab change sprintf to snprintf and test for buffer overflow */
  if (snprintf(date_string, date_stringsz, "%2d %.3s %4d", (int)day, month_names[mon - 1],(int)year) >= date_stringsz){
     k_error("Overflowed date_string in op_timedate()."); 
  }
